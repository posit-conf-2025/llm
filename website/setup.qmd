...
![Fighting with your IT department during our workshop is not fun.](assets/frustrated-man.png){alt="Frustrated man holding his head in his hands and looking confused at his laptop."}
:::
:::
...
For the workshop, I'd recommend installing `granite3.2-vision`[^plus-more].

[^plus-more]: If you want another, slightly bigger, model to try out, add [gpt-oss:20b][ollama-gpt-oss] or a `qwen3` variant, either [qwen3:8b][ollama-qwen3-8b] or [qwen3:14b][ollama-qwen3-14b].

| ID | Size | Capabilities |
|--:|--:|:--|
| [granite3.2-vision][ollama-granite-vision] | 2.4GB | [tools]{.badge .text-bg-primary} [vision]{.badge .text-bg-danger} |
...
| [gemma3:4b](https://ollama.com/library/gemma3:4b) | 3.3GB | [vision]{.badge .text-bg-danger} |

To use a model, you first need to download it.
For example, to download [granite3.2-vision][ollama-granite-vision], run the following command in your terminal:
...
chat <- chat_ollama(model = "granite3.2-vision")
...
chat = ChatOllama(model="granite3.2-vision")
chat.chat("What is the capital of France?")
```
...
**Local models are no where near as capable as the flagship models from OpenAI or Anthropic.**
Still, there are a few reasons you might want to use a local model:
